...
def minimum_bound(p_x, p_y, a_x, a_y):
    if min(a_x) < p_x < max(a_x) and min(a_y) < p_y < max(a_y):  # is the point within the bounds of the box
        return True
    elif min(a_x) == p_x and min(a_y) <= p_y <= max(a_y):  # Point is on the boundary of the mbr
        return 1
    elif max(a_x) == p_x and min(a_y) <= p_y <= max(a_y):  # Point is on the boundary of the mbr
        return 1
    elif min(a_y) == p_y and min(a_x) <= p_x <= max(a_x):  # Point is on the boundary of the mbr
        return 1
    elif max(a_y) == p_y and min(a_x) <= p_x <= max(a_x):  # Point is on the boundary of the mbr
        return 1
    else:
...
def is_on_line(p_x, p_y, a_x, a_y, b_x, b_y):
    if not ((a_y <= p_y <= b_y or b_y <= p_y <= a_y) and (a_x <= p_x <= b_x or b_x <= p_x <= a_x)):
        return False                 # The point lies within range
...
# Returns the result of a point on line equation
...
# Same as "locate_points" function but only takes the x and y values of one point.
