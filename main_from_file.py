...
def minimum_bound(p_x, p_y, a_x, a_y):
    if min(a_x) < p_x < max(a_x) and min(a_y) < p_y < max(a_y):  # is the point within the bounds of the box
        ...
        return 1
    elif max(a_x) == p_x and min(a_y) <= p_y <= max(
            a_y):  # if point x is equal to maximum x value, and between y values
        return 1
...
def ray_casting(pt,
                pg):  # Takes arguments pt - Point[x,y] and sh - polygon [x,y] - Generated by the generate_coordinates()
    n = len(pg) - 1
...
class ReadFile:
    ...
    def create_points(self):  # Splits data into two lists of x and y for points
